...
function chooseMostRelevant(child_divs, interestSignal, stoplist, classifier){
	...
	console.log('child_divs length is: ', child_divs.length)
	...
	for (let c in child_divs) {
		...
		let classification = classifier.classify(url=false, title=plain_text)
		
		console.log('child id: ', child_id, ' has classification: ', classification)
...
function process_page(containers){
	...
	for (let container_id of Object.keys(containers)) { //list of div containers
		...
		let most_relevant = chooseMostRelevant(containers[container_id], signal, stoplist=lica.stoplist, lica)
